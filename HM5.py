...
def plot_z_values(x, y, z, filename):
    ...
    for xi, yi, zi in zip(x, y, z):
        ax.text(xi, yi, str(zi), ha='center', va='center')
...
def generate_heatmap():
    ...
    if show_z_values.get():
        plot_z_values(x, y, z, os.path.join(os.path.expanduser("~"), "Desktop", f"{saved_image_name}_only_z_values.png"), font_color=font_color.get())
...
min_colormap_range_label = tk.Label(root, text="Laveste på baren:")
...
max_colormap_range_label = tk.Label(root, text="Høyeste på baren:")
...
show_z_values_checkbutton.grid(row=23, column=0, columnspan=2)
...
save_heatmap_only_checkbutton.grid(row=24, column=0, columnspan=2)

tk.Button(root, text="Preview", command=preview_heatmap).grid(row=22, column=0, columnspan=2)


heatmap_button = tk.Button(root, text="Generate Heatmap", command=generate_heatmap)
heatmap_button.grid(row=25, column=0, columnspan=2, pady=15)
