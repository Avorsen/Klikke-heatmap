...
class BoundaryClickHandler:
    def __init__(self, ax, coordinates):
        ...
        self.is_drawing = False  # Flag to detect if mouse button is being held
    ...
    def on_press(self, event):
        if self.boundary_closed or event.inaxes != self.ax:
            return
        
        # Start drawing mode when the mouse button is pressed
        self.is_drawing = True
        self.add_point(event)  # Add a point when clicking or starting to drag

    def on_release(self, event):
        # Stop drawing mode when the mouse button is released
        self.is_drawing = False

    def on_motion(self, event):
        # While holding the button, continue to add points as the mouse moves
    ...
    def add_point(self, event):
        ...
        # Find the closest point in the coordinates
        closest_coord = min(self.coordinates, key=lambda coord: (coord[0] - x)**2 + (coord[1] - y)**2)

        # Ensure that we don't immediately close the boundary if it's the first point being added
        if len(self.boundary) > 1 and closest_coord == self.boundary[0]:
            self.boundary_closed = True
            print("Boundary has been set.")
            # Draw a line from the last point to the first to close the loop
        ...
        # Avoid adding the same point consecutively or if the boundary is closed
        if not self.boundary or (self.boundary[-1] != closest_coord and not self.boundary_closed):
            self.boundary.append(closest_coord)
            self.ax.plot(*closest_coord, 'ro')  # Plot the boundary point

            if len(self.boundary) > 1:
                # Draw a line between the last two points
                self.ax.plot(*zip(*self.boundary[-2:]), color='r')
...
def upload_image():
    ...
    ax.text(.01, 0.99, '1. Left-click (-)',
            transform=ax.transAxes, color='green', fontsize=14, va='top')
    ax.text(0.01, 0.97, '2. Right-click (+)',
            transform=ax.transAxes, color='green', fontsize=14, va='top')
    ax.text(0.01, 0.95, '3. Shift + Venstreklikk (erase point)',
            transform=ax.transAxes, color='green', fontsize=14, va='top')
...
plt.matplotlib.colormaps.register(cmap=cmap)
...
plt.matplotlib.colormaps.register(cmap=cmap2)
...
plt.matplotlib.colormaps.register(cmap=new_cmap)
...
plt.matplotlib.colormaps.register(cmap=opaque_cmap_custom)
...
plt.matplotlib.colormaps.register(cmap=new_cmap2)
...
def plot_z_values(x, y, z, filename):
    fig, ax = plt.subplots()
    
    for xi, yi, zi in zip(x, y, z):
        z_value_str = ('+' if zi > 0 else '') + str(int(zi)) if float(zi).is_integer() else str(zi)

        if outline_enabled.get():  # Apply outline if enabled
            outline_color = 'white'
            strength = outline_strength.get()  # Get outline strength from UI
            
            # Define offsets for the outline effect
            offsets = [
                (-strength, 0), (strength, 0), (0, -strength), (0, strength), 
            ...
            for dx, dy in offsets:
                ax.text(
                    xi + dx, yi + dy, z_value_str,
                    fontsize=int(font_size.get()), ha='center', va='center',
                    color=outline_color, alpha=0.9
                )

        # Draw the main text in the desired color
        ax.text(
            xi, yi, z_value_str,
            fontsize=int(font_size.get()), ha='center', va='center',
...
def generate_heatmap():
    ...
    # Save the figure with heatmap and z values
    i = 1
    while True:
        ...
        for x_coord, y_coord, z_value in zip(x, y, z):
            ...
            if outline_enabled.get():  # Check if the outline toggle is active
                outline_color = 'white'
                strength = outline_strength.get()  # Get the current value from the slider
                
                # Use the strength variable to adjust the offsets
                offsets = [
                    (-strength, 0), (strength, 0), (0, -strength), (0, strength), 
                    (-strength, -strength), (strength, strength), 
                    (-strength, strength), (strength, -strength)
                ...
                for dx, dy in offsets:
                    ax.text(
                        x_coord + dx, y_coord + dy, z_value_str,
                        fontsize=int(font_size.get()), ha='center', va='center',
                        color=outline_color, alpha=0.9
                    )
            ...
            # Draw the main text in the desired color
            ax.text(
                x_coord, y_coord, z_value_str,
                fontsize=int(font_size.get()), ha='center', va='center',
                color=font_color.get()
            )
    plt.savefig(os.path.join(os.path.expanduser("~"), "Desktop", f"{saved_image_name}.png"), dpi=300, bbox_inches='tight', pad_inches=0)
    
    # Save transparent image with only z values
    if show_z_values.get():
        plot_z_values(x, y, z, os.path.join(os.path.expanduser("~"), "Desktop", f"{saved_image_name}_only_z_values.png"))

    # Save heatmap only without z values
    if save_heatmap_only.get():
        fig, ax = plt.subplots(figsize=(10, 10))
        im = ax.imshow(masked_zi, extent=(min(x), max(x), min(y), max(y)), cmap=selected_colormap.get(), origin='lower', vmin=min_colormap_range, vmax=max_colormap_range)
        ax.invert_yaxis()  # Invert the y-axis for correct heatmap orientation
        ...
        plt.savefig(os.path.join(os.path.expanduser("~"), "Desktop", f"{saved_image_name}_heatmap_only.png"), dpi=300, bbox_inches='tight', pad_inches=0)
        plt.close(fig)

    print("Heatmap saved to Desktop. You can find the images with and without z-values, and dot on xy if ticked")

# Define the update_heatmap function
def update_heatmap(*args):
    preview_heatmap()

# Function to debounce the heatmap update
def debounce_update_heatmap(delay=500):
    if hasattr(debounce_update_heatmap, 'after_id'):
        root.after_cancel(debounce_update_heatmap.after_id)
    debounce_update_heatmap.after_id = root.after(delay, update_heatmap)


def on_preview_window_close():
    global preview_window
    if preview_window is not None:
        preview_window.destroy()  # This closes the window
        preview_window = None     # Reset preview_window to None after closing


def preview_heatmap():
    global preview_window
    
    # Create a new window or clear the existing window if it already exists
    if preview_window is None or not preview_window.winfo_exists():
        preview_window = tk.Toplevel()
        preview_window.title("Heatmap Preview")
        preview_window.protocol("WM_DELETE_WINDOW", on_preview_window_close)  # Ensure window close is handled properly
    else:
        for widget in preview_window.winfo_children():
            widget.destroy()

    # Call generate_heatmap and capture the figure and axes
    x, y, z, xi, yi, masked_zi = calculate_interpolation_data()
...
coordinates_text.grid(row=3, column=1, pady=20)
...
min_colormap_range_label = tk.Label(root, text="Høyeste punkt på baren:")
...
max_colormap_range_label = tk.Label(root, text="Laveste setning på baren (-):")
...
font_color_label.grid(row=13, column=0, sticky='e')
...
color_menu.grid(row=13, column=1, sticky='w')

# GUI setup for the outline toggle text
outline_toggle_label = tk.Label(root, text="Outline:")
outline_toggle_label.grid(row=12, column=0, sticky='e')

# Variable to track the outline toggle state
outline_enabled = tk.BooleanVar(value=True)  # Default to True (outline enabled)
outline_toggle = tk.Checkbutton(root, variable=outline_enabled)
outline_toggle.grid(row=12, column=0)

# Variable to store the outline strength value
outline_strength = tk.DoubleVar(value=1.2)  # Default value, adjustable range

outline_strength_slider = tk.Scale(root, from_=0.5, to=3.0, resolution=0.1, orient='horizontal',
                                   variable=outline_strength)
outline_strength_slider.grid(row=12, column=1, sticky='w')

cmap_label = tk.Label(root, text="Choose Colormap:")
cmap_label.grid(row=14, column=0, sticky='e')
...
colormap_menu.grid(row=14, column=1, sticky='w')
...
interpolation_method_label.grid(row=15, column=0, sticky='e')
...
interpolation_method_menu.grid(row=15, column=1, sticky='w')
...
average_z_label.grid(row=20, column=0, pady=5, sticky='e')
...
calculate_z_button.grid(row=20, column=1, sticky='w', padx=5, pady=0)
...
show_z_values = tk.BooleanVar()
show_z_values_checkbutton = ttk.Checkbutton(root, text="Just the Z values", variable=show_z_values)
show_z_values_checkbutton.grid(row=27, column=0, columnspan=2)

save_heatmap_only = tk.BooleanVar()
save_heatmap_only_checkbutton = ttk.Checkbutton(root, text="Just the heatmap", variable=save_heatmap_only)
save_heatmap_only_checkbutton.grid(row=228, column=0, columnspan=2)

tk.Button(root, text="Preview", command=preview_heatmap).grid(row=10, column=1, columnspan=2)


heatmap_button = tk.Button(root, text="Generate Heatmap", command=generate_heatmap)
heatmap_button.grid(row=26, column=0, columnspan=2, pady=15)

# Trace changes to the relevant StringVars and BooleanVars
selected_colormap.trace_add("write", update_heatmap)
font_color.trace_add("write", update_heatmap)
contour_lines_enabled.trace_add("write", update_heatmap)
selected_interpolation_method.trace_add("write", update_heatmap)

# Trace changes to the Spinbox/entry widgets for colormap ranges and contour settings
min_colormap_range_entry.bind("<KeyRelease>", lambda event: debounce_update_heatmap())
max_colormap_range_entry.bind("<KeyRelease>", lambda event: debounce_update_heatmap())
contour_lines_width_entry.bind("<KeyRelease>", lambda event: debounce_update_heatmap())
contour_lines_num_entry.bind("<KeyRelease>", lambda event: debounce_update_heatmap())
font_size.trace_add("write", lambda *args: debounce_update_heatmap())
outline_enabled.trace_add("write", lambda *args: debounce_update_heatmap())
outline_strength.trace_add("write", lambda *args: debounce_update_heatmap())
